...
CREATE TABLE `construction_work_ticket` (
  `id` varchar(32) NOT NULL COMMENT '名称：主键id；',
  `ticket_type` varchar(2) NOT NULL COMMENT '名称：作业票类型；包括：A-A票；B-B票',
  `ticket_no` varchar(128) DEFAULT NULL COMMENT '名称：作业票编号；',
  `ticket_name` varchar(128) NOT NULL COMMENT '名称：作业票名称；',
  `bidding_section_code` varchar(32) DEFAULT NULL COMMENT '	标段编码		',
  `bidding_section_name` varchar(255) DEFAULT NULL COMMENT '	标段名称	标段名称	',
  `single_project_type` int(4) DEFAULT NULL COMMENT '	单项工程类型	包括：1-变电工程；2-线路工程；3-电缆工程	',
  `single_project_code` varchar(32) DEFAULT NULL COMMENT '	单项工程编码		',
  `single_project_name` varchar(255) DEFAULT NULL COMMENT '	单项工程名称		',
  `team_id` varchar(32) NOT NULL COMMENT '名称：班组id；',
  `working_team_name` varchar(128) NOT NULL COMMENT '名称：作业班组名称；',
  `construction_headcount` int(8) DEFAULT NULL COMMENT '	施工人数		',
  `planned_start_date` datetime DEFAULT NULL COMMENT '	计划开始时间		',
  `planned_end_date` datetime DEFAULT NULL COMMENT '	计划结束时间		',
  `start_time` datetime DEFAULT NULL COMMENT '名称：开始时间；',
  `end_time` datetime DEFAULT NULL COMMENT '名称：结束时间；',
  `assessment_risk_level` int(4) DEFAULT NULL COMMENT '	初勘风险等级	包括：2；3；4；5	',
  `re_assessment_risk_level` int(4) DEFAULT NULL COMMENT '	复测风险等级	包括：2；3；4；5	',
  `ticket_status` varchar(8) NOT NULL COMMENT '名称：作业票状态；包括：01-撤回；02-驳回；03-提交审核中；04-待执行；05-执行中；06-已结束；07-作废；08-删除',
  `current_construction_status` varchar(8) DEFAULT NULL COMMENT '	当日施工状态	包括：01-作业中；02-暂停中；03-作业完工	',
  `audit_hierarchy` varchar(8) DEFAULT NULL COMMENT '	审批层级	"包括：01-班组02-施工；03-监理；"	',
  `issue_type` varchar(8) DEFAULT NULL COMMENT '名称：签发类型；包括：01-审核签发；02-自动签发',
  `issue_date` datetime DEFAULT NULL COMMENT '名称：签发日期；',
  `priority` varchar(8) DEFAULT NULL COMMENT '名称：优先级；',
  `build_unit_code` varchar(32) NOT NULL COMMENT '名称：建设管理单位编码；',
  `province_code` varchar(32) DEFAULT NULL COMMENT '名称：省公司编码；',
  `remark` varchar(480) DEFAULT NULL COMMENT '名称：备注；',
  `huv_flag` int(4) DEFAULT '0' COMMENT '0:常规工程  1:特高压',
  `creater_id` varchar(32) NOT NULL COMMENT '名称：创建人；',
  `create_time` datetime NOT NULL COMMENT '名称：创建时间；',
  `updater_id` varchar(32) DEFAULT NULL COMMENT '名称：更新人；',
  ...
  `delete_flag` int(11) NOT NULL DEFAULT '0' COMMENT '名称：删除状态；默认0，包括：0未删除，1已删除',
  `voltage_level` varchar(12) DEFAULT NULL COMMENT '	电压等级		',
  `construction_unit_name` varchar(256) DEFAULT NULL COMMENT '施工单位名称',
  `construction_social_credit_code` varchar(256) DEFAULT NULL COMMENT '施工单位统一社会信用代码',
  `supervision_unit_name` varchar(256) DEFAULT NULL COMMENT '监理单位',
  `supervision_social_credit_code` varchar(256) DEFAULT NULL COMMENT '监理单位统一社会信用代码',
  PRIMARY KEY (`id`),
  KEY `sin_prj_code_index` (`single_project_code`) USING BTREE,
  KEY `bid_sect_code_and_ticket_status_index` (`bidding_section_code`,`ticket_status`) USING BTREE,
  KEY `build_unit_code_and_ticket_status_index` (`build_unit_code`,`ticket_status`) USING BTREE,
  KEY `create_time` (`create_time`) USING BTREE,
  KEY `team_id` (`team_id`) USING BTREE,
  KEY `ticket_team_del_index` (`team_id`,`delete_flag`) USING BTREE,
  KEY `construction_work_ticket_ticket_no_IDX` (`ticket_no`,`delete_flag`) USING BTREE
) ENGINE=InnoDB DEFAULT CHARSET=utf8 COMMENT='名称：施工作业票；';